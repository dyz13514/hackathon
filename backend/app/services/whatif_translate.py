...
from app.llm.adapter import BedrockAdapter, BedrockUnavailableError, LlmDisabledError, LlmRequest
...
def translate_whatif_query(
    ...
    **不执行**：返回的 `mutations` 是待确认载荷，执行由前端确认后交 `/api/scenarios/run`。
    ...
    for _step in range(MAX_TRANSLATE_STEPS):
        ...
        except (LlmDisabledError, BedrockUnavailableError):
            return TranslationResult(
                outcome=TranslationOutcome.LLM_UNAVAILABLE,
                injection_suspected=verdict.suspected,
                source_query_echo=query,
    ...
    # The model failed to produce valid output; this is not evidence that the user's
    # scenario is unsupported. Report the model failure explicitly.
    return TranslationResult(
        outcome=TranslationOutcome.LLM_UNAVAILABLE,
        injection_suspected=verdict.suspected,
        source_query_echo=query,
        reason="The model returned no valid scenario translation after three attempts.",
    )
